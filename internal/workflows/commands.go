...
func CreateSpec(projectRoot, description string) (core.SpecResult, error) {
	...
	filePath := filepath.Join(root, ".moai", "specs", specID+".md")
...
func CreateWorkflowArtifact(projectRoot, command, summary, specID string) (core.WorkflowResult, error) {
	root := core.FindProjectRoot(projectRoot)
	reportsDir := filepath.Join(root, ".moai", "project", "reports")
	if err := core.EnsureDir(reportsDir); err != nil {
		return core.WorkflowResult{}, err
	}
	if err := core.EnsureDir(filepath.Join(root, ".moai", "project", "codemaps")); err != nil {
		return core.WorkflowResult{}, err
	}
	...
	if command == "codemaps" {
		...
		if err := os.WriteFile(filepath.Join(root, ".moai", "project", "codemaps", "overview.md"), []byte(content), 0o644); err != nil {
			return core.WorkflowResult{}, err
		}
	}
...
func writeProjectDocs(root, summary string) error {
	if err := os.WriteFile(filepath.Join(root, ".moai", "project", "product.md"), []byte("# Product\n\n"+summary+"\n"), 0o644); err != nil {
		return err
	}
	if err := os.WriteFile(filepath.Join(root, ".moai", "project", "structure.md"), []byte("# Structure\n\n- Generated by moai-codex project workflow.\n"), 0o644); err != nil {
		return err
	}
	return os.WriteFile(filepath.Join(root, ".moai", "project", "tech.md"), []byte("# Tech\n\n- Runtime: Codex CLI\n- Compatibility layer: moai-codex\n"), 0o644)
}
...
func buildPrompt(command, summary, specID string) string {
	...
	return fmt.Sprintf("%s\n\nSummary: %s\n\nUse Codex/OMX-compatible behavior and preserve .moai artifacts.", header, summary)
}
